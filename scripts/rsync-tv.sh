...
if ping -c 1 192.168.1.37 > /dev/null; then
    echo "Beginning TV transfer" >> $OUTPUT
    rsync -avzI --delete-after --bwlimit=100000 --size-only --no-owner --no-group --omit-dir-times --human-readable --no-perms --fuzzy finish06@192.168.1.37:/mnt/media/tv/ /mnt/storage1/Media/TV/ >> $OUTPUT
else
    echo "TV transfer failed" >> $OUTPUT
fi
