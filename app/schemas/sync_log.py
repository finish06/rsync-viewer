...
class SyncLogRead(BaseModel):
    ...
    total_size_bytes: Optional[int] = Field(None, description="Total size of files in bytes")
    bytes_sent: Optional[int] = Field(None, description="Bytes sent during transfer")
    bytes_received: Optional[int] = Field(None, description="Bytes received during transfer")
    transfer_speed: Optional[float] = Field(None, description="Transfer speed in bytes/sec")
    ...
    is_dry_run: bool = Field(False, description="Whether this was a dry run (no actual transfer)")
...
class SyncLogDetail(SyncLogRead):
    ...
    file_list: Optional[list[str]] = Field(None, description="List of files that were transferred")
...
class PaginatedResponse(BaseModel):
    ...
    total: int = Field(..., description="Total number of matching records", examples=[42])
...
class ErrorResponse(BaseModel):
    ...
    detail: str = Field(..., description="Error message", examples=["Sync log not found"])
