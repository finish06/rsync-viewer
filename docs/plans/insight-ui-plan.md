...
| C6 | **code complete** — `/` served by `app/routes/spa.py` with `window.__USER__`/`__USER_THEME__` injected at the top of `<head>` (keeps the FOUC-prevention and role-aware menu without a round trip); legacy dashboard routes/templates/tests removed; `/notifications` server-rendered page keeps the HTMX history partial; E2E dashboard/analytics/login rewritten. Lesson: GitHub closes a stacked PR when its base branch is deleted — merge stacked PRs bottom-up or retarget before deleting. |
